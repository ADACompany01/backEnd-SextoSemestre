...
export class LighthouseController {
  ...
  async checkAccessibility(@Body('url') url: string) {
    console.log(
      `[LighthouseController] Recebida requisição para analisar: ${url}`,
    );
    ...
    try {
      console.log(
        `[LighthouseController] Iniciando análise do Lighthouse para: ${url}`,
      );
      const result = await this.lighthouseService.runLighthouse(url);
      const duration = Date.now() - startTime;
      console.log(
        `[LighthouseController] Análise concluída com sucesso em ${duration}ms`,
      );
      return result;
    } catch (error) {
      const duration = Date.now() - startTime;
      ...
      console.error(
        `[LighthouseController] Erro após ${duration}ms:`,
        errorMessage,
      );
      ...
      if (
        errorMessage.includes('não pôde ser acessada') ||
      ...
      if (
        ...
        console.error(
          `[LighthouseController] Erro crítico do Chromium: ${errorMessage}`,
        );
        throw new HttpException(
