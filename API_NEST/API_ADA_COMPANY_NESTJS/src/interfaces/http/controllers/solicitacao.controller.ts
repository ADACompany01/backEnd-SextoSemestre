...
export class SolicitacaoController {
  ...
  @Post()
  @ApiOperation({
    summary: 'Criar nova solicitação de orçamento (apenas clientes)',
  })
  @ApiResponse({
    status: 201,
  ...
  async create(
    ...
    try {
      ...
      const cliente = await this.getClienteByUsuarioUseCase.execute(
        user.id_usuario,
      );
      if (!cliente) {
        throw new HttpException(
  ...
  @Get()
  @ApiOperation({
    summary: 'Listar todas as solicitações (apenas funcionários)',
  })
  @ApiResponse({
    status: 200,
  ...
  async findMySolicitacoes(@Request() req: any) {
    try {
      ...
      const solicitacoes = await this.getSolicitacoesByClienteUseCase.execute(
        cliente.id_cliente,
      );
      return {
        statusCode: HttpStatus.OK,
      ...
      this.logger.error(
        `Erro ao listar solicitações do cliente: ${error.message}`,
        error.stack,
      );
      if (error instanceof HttpException) {
        throw error;
  ...
  @ApiOperation({
    summary:
      'Criar orçamento automaticamente a partir de uma solicitação (apenas funcionários)',
  })
  @ApiParam({ name: 'id', description: 'ID da solicitação' })
  @ApiResponse({
    status: 201,
  ...
  async createOrcamento(
    @Param('id') id: string,
    @Body() body?: { valor_orcamento?: number },
  ) {
    try {
      const orcamento =
        await this.createOrcamentoFromSolicitacaoUseCase.execute(
          id,
      ...
      this.logger.error(
        `Erro ao criar orçamento para solicitação ${id}: ${error.message}`,
        error.stack,
      );
      if (error instanceof HttpException) {
        throw error;
  ...
  @ApiOperation({
    summary:
      'Atualizar status de uma solicitação (clientes podem atualizar suas próprias solicitações)',
  })
  @ApiParam({ name: 'id', description: 'ID da solicitação' })
  @ApiResponse({
  ...
  async update(
    ...
    try {
      ...
      const updatedSolicitacao = await this.updateSolicitacaoUseCase.execute(
        id,
        updateDto,
      );
