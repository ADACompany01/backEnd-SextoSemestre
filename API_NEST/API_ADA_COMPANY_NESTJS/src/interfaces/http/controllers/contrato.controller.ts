...
import {
  CreateContratoDto,
  StatusContrato,
} from '../../../interfaces/http/dtos/requests/create-contrato.dto';
...
export class ContratoController {
  ...
  async create(@Body() createContratoDto: CreateContratoDto) {
    try {
      const contrato =
        await this.createContratoUseCase.execute(createContratoDto);
      return this.toContratoResponseDto(contrato);
    } catch (error) {
      this.logger.error(
        `Erro ao criar contrato: ${error.message}`,
        error.stack,
      );
  ...
  async findOne(@Param('id') id: string) {
    try {
      ...
      this.logger.error(
        `Erro ao buscar contrato por ID ${id}: ${error.message}`,
        error.stack,
      );
      // Aqui você pode adicionar lógica para tratar erros específicos dos use-cases,
  ...
  async update(
    ...
    try {
      ...
      const updatedContrato = await this.updateContratoUseCase.execute(
        id,
        updateContratoDto,
      );
      ...
      this.logger.error(
        `Erro ao atualizar contrato com ID ${id}: ${error.message}`,
        error.stack,
      );
      // Aqui você pode adicionar lógica para tratar erros específicos dos use-cases
      throw new HttpException(
  ...
  async uploadFile(@Param('id') id: string, @UploadedFile() file: any) {
    try {
      ...
      await this.updateContratoUseCase.execute(id, {
        arquivo_contrato: filePath,
      } as any);
      ...
      this.logger.error(
        `Erro ao fazer upload do arquivo do contrato ${id}: ${error.message}`,
        error.stack,
      );
      throw new HttpException(
        {
  ...
  async sign(@Body() signContratoDto: SignContratoDto) {
    try {
      ...
      this.logger.error(
        `Erro ao assinar contrato: ${error.message}`,
        error.stack,
      );
      throw new HttpException(
        {
  ...
  async remove(@Param('id') id: string) {
    try {
      ...
      this.logger.error(
        `Erro ao remover contrato com ID ${id}: ${error.message}`,
        error.stack,
      );
      // Aqui você pode adicionar lógica para tratar erros específicos dos use-cases,
      // como NotFoundException
