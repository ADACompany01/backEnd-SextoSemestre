...
export class OrcamentoRepository implements OrcamentoRepositoryInterface {
  ...
  async update(
    id: string,
    data: Partial<OrcamentoModel>,
  ): Promise<[number, OrcamentoModel[]]> {
    const [affectedCount, affectedRows] = await this.orcamentoModel.update(
      data,
      {
    ...
    const updatedOrcamentos = affectedRows.map(
      (orcamento) => orcamento.toJSON() as OrcamentoModel,
    );
